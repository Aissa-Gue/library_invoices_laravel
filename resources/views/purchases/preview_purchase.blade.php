    ...
    <fieldset class="scheduler-border">
        ...
        <div class="row mt-3">
            <div class="col-md-4">
                <p>
                    <strong>الزبون:
                    </strong>{{ $purchase->provider->person->last_name .' ' .$purchase->provider->person->first_name .' بن ' .$purchase->provider->person->father_name }}
                </p>
            </div>
            @if ($purchase->provider->establishment)
                <div class="col-md-4">
                    <p>
                        <strong>المؤسسة:
                        </strong>{{ $purchase->provider->establishment }}
                    </p>
                </div>
            @endif
            <div class="col-md-4">
        ...
        <div class="row mt-3">
            <div class="col-md-4">
                <p class="text-success fw-bold">
                    <strong class="text-dark me-2">البائع: </strong> <i class="fas fa-user-shield"></i>
                    {{ $purchase->createdBy->username }}
                </p>
            </div>
            <div class="col-md-4">
                <p class="text-success fw-bold">
                    <strong class="text-dark me-2">المعدل: </strong> <i class="fas fa-user-shield"></i>
    ...
    <fieldset class="scheduler-border">
                    ...
                    @foreach ($purchaseBooks as $purchaseBook)
                        <tr>
                            ...
                            <td class="text-center">
                                {{ number_format($purchaseBook->purchase_price + ($purchaseBook->purchase_price * $purchaseBook->book->sale_percentage) / 100,2) }}
                            </td>
                            <td class="text-center">{{ $purchaseBook->quantity }}</td>
                            <td class="text-center">
                                {{ number_format($purchaseBook->purchase_price * $purchaseBook->quantity, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @endphp
                            <th class="text-center text-danger @if ($debt != 0) hvr-pop @endif">
                                {{ number_format($debt, 2) }}</th>
                        </tr>
                        <tr>
        ...
        <div class="float-end mt-2">
            <form action="{{ Route('deletePurchase', $purchase->id) }}" method="post">
                @csrf
                @method('DELETE')
                <a href="{{ Route('editPurchase', $purchase->id) }}" class="btn btn-primary w-25">
                    <i class="fas fa-edit"></i>
                    تعديل
                </a>
                <button type="submit" class="btn btn-danger btn-r w-25" onclick="return confirm('هل أنت متأكد؟')"><i
