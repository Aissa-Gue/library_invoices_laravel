...
@section('content')
        ...
        <div class="row my-3">
            <div class="col-md-5">
                <div class="input-group">
                    ...
                    <input
                        value="{{ $order->client->person->last_name .' ' .$order->client->person->first_name .' بن ' .$order->client->person->father_name }}"
                        class="form-control" name="client_id" id="client_id" placeholder="أدخل اسم الزبون" disabled>
                </div>
            </div>
        </div>
    </fieldset>
    ...
    <fieldset class="scheduler-border">
        ...
        @foreach ($orderBooks as $orderBook)
            <div class="row mt-1">
                <div class="col-md-5">
                    <input value="{{ $orderBook->book->title }}" class="form-control" id="title" disabled>
                </div>

                <div class="col-md-2">
                    <input type="number" class="form-control text-center" id="quantity" value="{{ $orderBook->quantity }}"
                        disabled>
                </div>
                ...
                <div class="col-md-1">
                    <form action="{{ Route('deleteOrderBook', [$order->id, $orderBook->book_id]) }}" method="post">
                        @csrf
                        @method('DELETE')
