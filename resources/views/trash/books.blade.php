...
@section('content')
    ...
    <div class="row mt-3">
        <div class="col-md-12">
            <h4><i class="fas fa-book-open"></i> الكتب المحذوفة</h4>
            <hr>
        </div>
        ...
        <div class="col-md-12">
            <table class="table table-hover">
                ...
                @endif
                @foreach($trashedBooks as $trashedBook)
                    <tr class="fw-bold">
                        <th class="text-danger"><i class="fas fa-arrow-alt-circle-left fs-5"></i></th>
                        <td>{{$trashedBook->title}}</td>
                        <td>{{$trashedBook->author}}</td>
                        <td>{{$trashedBook->deleted_at}}</td>
                        <td class="text-center">
                            <form action="{{route('restoreTrashedBook', $trashedBook->id)}}" method="POST">
                                @csrf
                        ...
                        <td class="text-center">
                            <form action="{{route('deleteTrashedBook', $trashedBook->id)}}" method="POST">
                                @csrf
                                @method('DELETE')
