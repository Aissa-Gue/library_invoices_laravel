...
@section('content')
    ...
    <div class="alert alert-primary text-center" role="alert">
        <h4>قائمة المزودين</h4>
    </div>
    <div class="row justify-content-sm-center mb-3">
        <div class="col-sm-7">
            <form action="{{ Route('providersList') }}" method="get">
    ...
    <div class="row">
        <div class="alert alert-warning text-center" role="alert">
            <strong> عدد النتائج = </strong> {{ $providers->total() }}
        </div>

        <table class="table table-hover">
            <thead>
                <tr>
                    <th scope="col" class="text-center">رقم المزود</th>
            ...
            <tbody>
                ...
                @foreach ($providers as $provider)
                    <tr>
                        ...
                        <td>{{ $provider->last_name . ' ' . $provider->first_name . ' بن ' . $provider->father_name }}
                        </td>
                        <td>{{ $provider->establishment }}</td>
                        <td class="text-center">0{{ $provider->phone1 }}</td>

                        <td class="text-center">
                            <a class="btn btn-outline-dark" href="{{ URL('purchases/add/' . $provider->id) }}">
                        ...
                        <td class="text-center">
                            <a class="btn btn-outline-success" href="{{ route('previewProvider', $provider->id) }}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        ...
                        <td class="text-center">
                            <a class="btn btn-outline-primary" href="{{ route('editProvider', $provider->id) }}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
