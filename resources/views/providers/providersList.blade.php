        ...
        <table class="table table-hover">
            ...
            @foreach($providers as $provider)
                <tr>
                    ...
                    <td>{{$provider->last_name .' '. $provider->first_name .' بن '.$provider->father_name}}</td>
                    <td>{{$provider->establishment}}</td>
                    <td class="text-center">0{{$provider->phone1}}</td>

                    <td class="text-center">
                        <a class="btn btn-outline-dark"
                           href="{{URL('purchases/add/'.$provider->id)}}">
                    ...
                    <td class="text-center">
                        <a class="btn btn-outline-success"
                           href="{{route('previewProvider', $provider->id)}}">
                    ...
                    <td class="text-center">
                        <a class="btn btn-outline-primary"
                           href="{{route('editProvider', $provider->id)}}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                    ...
                    <td class="text-center">
                        @if(Auth::user()->role == 'admin')
                            <a class="btn btn-outline-danger"
                               href="{{route('deleteProvider', $provider->id)}}"
