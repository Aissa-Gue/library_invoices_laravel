        ...
        <div class="col-sm-7">
            <form action="{{ Route('addSale') }}" method="get">
                <div class="input-group mb-3">
                    ...
                    <input type="date" name="created_at" class="form-control" placeholder="أدخل تاريخ البيع"
                        value="{{ request('created_at') ?? date('Y-m-d') }}">
        ...
        <tbody>
            @if ($sales->isEmpty())
                <tr class="fw-bold text-center text-danger">
                    <td colspan="7" class="py-4"><i class="fas fa-exclamation-triangle fs-4 mb-3"></i><br> لا
                        توجد مبيعات !</td>
                </tr>
            @endif
            @foreach ($sales as $sale)
                <tr>
                    ...
                    <td class="fw-bold text-center text-success"><i class="fas fa-user-shield"></i>
                        {{ $sale->createdBy->username }}</td>
                    <td class="fw-bold text-center">{{ $sale->quantity }}</td>
                    <td class="text-success fw-bold text-center">
                        {{ number_format($sale->sale_price * $sale->quantity, 2) }}
                        دج
                    </td>
